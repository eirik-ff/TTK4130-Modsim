...
state = [0;0;0];  % euler angles
omega_ab_in_b = 2 * [1; 1; 1];
...
try
    ...
    [time,statetraj] = ode45(@(t,x)Kinematics(t, x, omega_ab_in_b),[0,time_final],state);
...
while time_display < time(end)
    ...
    omega = omega_ab_in_b;
    R = Rotations(state_animate.');  % .' to avoid complex conjugates
