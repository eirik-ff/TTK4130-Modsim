...
m = M + m0;
r_oc_b = 0.2 * ones(3,1);
Mbc = M * l^2 / 6 * eye(3) + m * ( (r_oc_b' * r_oc_b) * eye(3) - r_oc_b * r_oc_b' );
...
parameters = {1, 1, 1, Mbc}; %{G, m_T, r_c};
...
[time,statetraj] = ode45(@(t,x)SatelliteDynamics_b(t, x, parameters),[0,time_final],state);
...
while time_display < time(end)
    ...
    MakeFrame(p + r_oc_b,R,ScaleFrame,FS,SW,'b', 'color', 'r')
    MakeArrow(p + r_oc_b,R*omega,FS,SW,'$$\omega$$', 'color', [0,0.5,0])
