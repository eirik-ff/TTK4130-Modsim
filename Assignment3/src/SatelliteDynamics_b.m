function [ state_dot ] = SatelliteDynamics_b( t, state, parameters )
% parameters = {G, m_T, norm_r_c_in_a, Mbc}
% 
% state = [r_c;
%          R_ba;
%          v_c;
...
omega_coeff = -inv(Mbc) * skewsym3x3(omega_ab_in_b) * Mbc;

M = [zeros(3),            eye(3),   zeros(3);
     grav_force * eye(3), zeros(3), zeros(3);
     zeros(3),            zeros(3), omega_coeff];
